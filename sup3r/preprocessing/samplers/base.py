...
class Sampler(Container):
    ...
    def _append_obs_features(self, samples):
        """Append or merge observation features into the batch samples.

        When real obs are present in the data they are combined with proxy
        obs: real obs values are kept where they are not NaN, and proxy obs
        fill the remaining locations.
        ...
        -------
        samples : np.ndarray | tuple[np.ndarray, ...]
            Same as input, but with obs features appended/merged on the
            last dimension if proxy obs are enabled.
        """
        if not self.use_proxy_obs:
            return samples
        ...
        if isinstance(samples, tuple):
            hr = samples[-1]
            obs = self._merge_obs(hr)
            hr = np.concatenate([hr, obs], axis=-1)
            return (*samples[:-1], hr)

        obs = self._merge_obs(samples)
        return np.concatenate([samples, obs], axis=-1)

    def _merge_obs(self, hi_res):
        """Generate merged observation array from proxy and (optionally)
        real obs. Where real obs are not NaN they take priority over the
        proxy values. Handles partial real obs: only obs features present
        in the data are merged; others remain as pure proxy.

        Parameters
        ----------
        hi_res : np.ndarray
            High-resolution sample array with shape
            (batch_size, s1, s2, t, n_features).

        Returns
        -------
        np.ndarray
            Merged obs array with shape
            (batch_size, s1, s2, t, n_obs_features).
        """
        proxy = self._get_proxy_obs(hi_res)
        if not self.real_obs_features:
            return proxy

        merged = proxy.copy()
        for i, feat in enumerate(self.obs_features):
            if feat in self.real_obs_features:
                real_ind = self.hr_sample_features.index(feat)
                real_vals = hi_res[..., real_ind]
                valid = ~np.isnan(real_vals)
                merged[..., i] = np.where(valid, real_vals, proxy[..., i])
        return merged
    ...
    @cached_property
    def real_obs_features(self):
        """Obs features that actually exist in the data. In contrast to proxy
        obs, which are generated by masking the gridded data, these features
        are present in the raw data and are sampled directly. These are a
        subset of obs_features."""
        return [
            f for f in self.obs_features if f in lowered(self.data.features)
        ]

    @cached_property
    def hr_sample_features(self):
        """List of feature names used in the sample index for the
        high-resolution training data. When using proxy obs, obs features
        that are not present in the raw data are excluded from sampling
        (they will be generated as proxy obs). Obs features that ARE in the
        data are included so they get sampled."""
        if self.use_proxy_obs:
            missing_obs = set(self.obs_features) - set(self.real_obs_features)
            return [f for f in self.hr_source_features if f not in missing_obs]
        return self.hr_source_features
    ...
    def obs_features_ind(self):
        """Get the source feature indices for each obs feature's
        corresponding base feature (e.g. ``u_100m`` for ``u_100m_obs``).
        Used to extract gridded truth for proxy obs generation.

        Returns
        -------
        list[int]
            Indices into ``hr_sample_features`` for each obs base feature.
        """
        base_feats = [f.replace('_obs', '') for f in self.obs_features]
        return [self.hr_sample_features.index(f) for f in base_feats]
